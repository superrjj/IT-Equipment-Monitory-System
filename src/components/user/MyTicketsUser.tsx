...
import { Ticket, Search, Eye, X, Clock } from "lucide-react";
...
const MyTicketsUser: React.FC = () => {
  ...
  const fetchTickets = async () => {
    ...
    const [{ data: reports, error }, { data: depts }] = await Promise.all([
      supabase
        .from("file_reports")
        .select(
          "id,ticket_number,title,issue_type,status,date_submitted,action_taken,started_at,completed_at,department_id,employee_name"
        )
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r =>
      [r.title, r.issue_type, r.status, r.ticket_number ?? ""].join(" ").toLowerCase().includes(q)
    );
  }, [rows, search]);
  ...
  return (
    <>
      <style>{`
        ...
        /* Same width as dashboard: only UserShell applies horizontal padding (10%). */
        .mtu-shell {
          width: 100%;
        }
        .mtu-topbar {
              ...
              <div className="mtu-grid">
                {filtered.map(r => (
                  <div key={r.id} className="mtu-ticket-card">
                    {/* Top area: ticket number like Shopee card style */}
                    <div
                      style={{
                    ...
                    {statusSteps.map((step, idx) => {
                      ...
                      return (
                        <div
                          ...
                          style={{
                            position: "relative",
                            paddingBottom: idx === statusSteps.length - 1 ? 0 : 12,
                          }}
                        >
                          <div
                            style={{
