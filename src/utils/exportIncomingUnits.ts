...
export async function exportIncomingUnitsToWord(
  ...
  const TABLE_W = 15840;
  const COL_W   = [1600, 2800, 2600, 3040, 2600, 3200];
            ...
            ...chunk.map((r) =>
              new TableRow({
                ...
                children: [
                  ...
                  dCell(r.unit_name,                                       COL_W[1], { bold: true, color: BRAND_HEX }),
                  dCell(r.reported_by,                                     COL_W[2]),
                  dCell(depts[r.department_id ?? ""]         ?? "—",      COL_W[3]),
                  dCell(userMap[r.received_by_user_id ?? ""] ?? "—",      COL_W[4]),
                  dCell(r.issue_description || "—",                        COL_W[5]),
                ],
              })
            ),
          ],
